...
//   1) <svg id="chart"></svg>
//   2) <div id="line-tooltip"></div>
//   3) d3 v7: <script src="https://d3js.org/d3.v7.min.js"></script>
...
(() => {
  ...
  d3.csv(CSV_PATH).then(rows => {
    ...
    // ---- End labels (right side) with offsets ----
    const lastYear = d3.max(allYears);
    const labelGroup = svg.append("g");

    function labelOffsetForSeries(s) {
      // Change names here if your series keys differ
      const BY_KEY = {
        "Investigation": -12,        // ⬆️ above orange line
        "Family Preservation": 12    // ⬇️ below blue line
      };
      if (Object.prototype.hasOwnProperty.call(BY_KEY, s.key)) return BY_KEY[s.key];

      // Fallback by color (Tableau10 palette)
      const hex = d3.color(color(s.key))?.formatHex().toLowerCase();
      if (hex === "#f28e2b") return -12; // orange
      if (hex === "#4e79a7") return 12;  // blue
      return 0;
    }
